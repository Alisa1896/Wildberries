...
// для получения данных, можно использовать в других проектах
const getGoods = async () => {
	const result = await fetch('db/db.json');
	if (!result.ok) {
		throw 'wrong' + result.status
	}
...
const filteredCards = function(field, value) {
	getGoods()
...
showAcsessories.forEach(item => {
	item.addEventListener('click', function(event) {
		event.preventDefault();
		filteredCards('category', 'Accessories');
	});
});
...
showClothing.forEach(item => {
	item.addEventListener('click', function(event) {
		event.preventDefault();
		filteredCards('category', 'Clothing');
	});
});